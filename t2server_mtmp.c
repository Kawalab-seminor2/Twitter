...
int main()          //メイン関数
{
    ...
    int     userpid[2]={0,0};
    char u1id[20],u2id[20],u3id[20],u4id[20],u5id[20];
    char u1ps[20],u2ps[20],u3ps[20],u4ps[20],u5ps[20];
    char    *oripid;
    ...
    while(1) {          //dataの内容を常に監視
    	...
    	if(strncmp(data, "pid,", 4)==0){        //pidを受け取ると
            ...
            if(uhead==NULL){
              ...
              sleep(1);
              //strcmpでユーザIDとパスワードを比較
              //未登録の場合は入力されたものを登録してログイン
              //2～5でも同様
              if(u1id==NULL){
                strcpy(u1id,data);
                sleep(1);
       ...
       //printf("%d\n",number);
		...
		alltweet = (char**)malloc(sizeof(char*) * sizeof(words11));
                strcpy(tweet1, words11[2]);
		
		
		alltweet=(alltweet+tweetnumber);
                sprintf(alltweet,"%s%s",u1t,tweet1);
		if(tweetnumber==0)
			toptweet=alltweet;
